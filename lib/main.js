...
module.exports = {
  config: {
    enabled: {
      title: "Enable the linter",
      description: "Toggles whether to enable linting or not, useful for enabling or disabling per-directory or per-file linting.",
      type: "boolean",
      default: true,
      order: 1
  },
  ...
  lint: function(editor, linted_file, real_file){
    ...
    const regex = "\\[.+\\] (?<type>(ERROR|WARNING|INFO|DEBUG)): (" + regexAS + "|" + regexLua +"|(?<message>.+))";
